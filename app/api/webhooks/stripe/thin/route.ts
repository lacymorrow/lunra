...
export async function POST(request: NextRequest) {
	...
	try {
		...
		console.error('[webhook-thin] Signature verification failed:', err)
		return NextResponse.json({ error: 'Invalid signature' }, { status: 400 })
	}

	try {
		await handleStripeEvent(event, 'webhook-thin')
		return NextResponse.json({ received: true, endpoint: 'thin' })
	} catch (error) {
		console.error('[webhook-thin] Error processing event:', error)
